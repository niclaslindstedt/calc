...
//
//   • a script that defines `window.__calcICloud`, whose methods post a
...
//
// The wrapper decides nothing about the sessions. It moves opaque files
// between the page and a folder in iCloud Drive: what a session file is called,
// which folder it is filed under and when a save is due are all the web app's,
// in `src/app/store.ts` against the framework's storage seam. A wrapper that
// parsed a session would be a second implementation of the app's domain.
//
...
export const ICLOUD_REQUEST_TYPE = "calc-native/icloud-request";

/** The seven things the page may ask for — mirrors `ICloudHost`.
 *
 *  Text and bytes are separate pairs on purpose. A session is markdown and
 *  crosses as text; a binary file would cross as base64,
...
export const ICLOUD_SCRIPT = `(function () {
  ...
  window.__calcICloudResolve = function (id, result) {
 ...
 * sessions: a session's note is arbitrary user text, and text is exactly
...
export function resolveScript(
  ...
  return `(function () {
    try {
      ...
      if (window.__calcICloudResolve) {
        window.__calcICloudResolve(answer.id, answer.result);
      }
    } catch (e) {}
  })(); true;`;
}
