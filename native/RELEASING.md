...
The app declares one iCloud container, `iCloud.se.agilator.calc`.
...
- [ ] `EXPO_PUBLIC_CALC_URL` is **unset** — a build that streams the
...
- [ ] On a real device signed in to iCloud: Settings → Storage offers **iCloud
      Drive**, connecting files the saved sessions under `calculations/` in
      the **Calc** folder in the Files app, and a session saved on one device
      shows up on another.
