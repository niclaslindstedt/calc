...
A **thin** Expo / React Native shell around the calc web app, so it can ship
to the App Store and Google Play — and so it can do the one thing a PWA cannot:
keep the saved sessions in the reader's own **iCloud Drive**.
...
iCloud **capability** on `window` and this installs one, so in a browser
(which has none) the iCloud option in the Storage picker explains that it
belongs to the App Store app. The app never asks what it is running inside.

The wrapper also decides nothing about the sessions. It moves opaque files
between the page and a folder. What a session file is called, which folder
it is filed under and when a save is due are all the web app's, in
`src/app/store.ts` and `src/app/useSessions.ts` — exactly as they are for a
picked local folder.
...
```sh
EXPO_PUBLIC_CALC_URL=https://calc.niclaslindstedt.se/preview/ npm run ios
```
...
Settings → Storage → iCloud Drive
   │  src/app/store.ts  — icloudFileStore(host), a FileStore over the host
   ▼
window.__calcICloud        — installed by src/icloudBridge.ts
   │  postMessage (request)  /  injectJavaScript (answer)
   ▼
App.tsx → src/icloud.ts → modules/icloud-store
   │
   ▼
iCloud.se.agilator.calc/Documents/
   calculations/<folder>/<session>.md, calculations/folders.json,
...
up under **Calc** in the Files app. A file the reader can see is a file they
...
The bridge carries paths and file contents, and nothing else. Sessions are
markdown and cross as text. (The bridge also has a base64 channel for binary
files, which calc does not use.) Nothing is cached on the
native side and nothing is logged — the payload is somebody's sessions, and
...
`http://localhost:8261…`, an origin it has not registered, in a browser that
...
Settings → Storage → Dropbox → Connect
   │  src/app/useSessions.ts — getAuthSessionHost() is present, so
...
- **The port in `src/local-server.ts` is fixed on purpose.** A web origin is
  ...
  the working tape and every session kept on the device would appear to
  vanish.
