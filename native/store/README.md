...
Everything App Store Connect needs for Calc's product page, compiled from one
...
Calc ships on the **App Store** alone — iPhone and iPad. `listing.mts`
...
Taken of **the real app on its demo data**: `VITE_SEED=demo` builds Calc onto
a shelf of eight saved sessions held in memory (`src/app/dev/demoData.ts`;
