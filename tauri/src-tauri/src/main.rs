...
//! The Calc desktop shell — a window showing the bundled site, and nothing
//! else.
//!
...
//! here needed no change to the app at all. Everything Calc does — the
...
use calc_shell::config::APP_SCHEME;
use calc_shell::oauth::{AWAIT_PATH, BEGIN_PATH};
use calc_shell::webroot::webroot_exists;
...
fn main() {
    tauri::Builder::default()
        ...
        // `calc_shell::webroot` for the three properties this arrangement is
        ...
        // `calc_shell::oauth`), which can be minutes, so it answers from a
        ...
        .setup(|app| {
            ...
            if calc_shell::config::remote_app_url().is_none() && !webroot_exists(&root) {
                let message = format!(
                    "Calc could not find the app it is meant to show.\n\n\
                     Looked in: {}\n\n\
        ...
        .expect("the Calc desktop shell could not start");
}
