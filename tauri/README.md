...
# Calc — the desktop app

A desktop wrapper around Calc for **Windows, macOS and Linux**. It is a thin
...
minimum. Everything Calc does — the document, the settings, the sync engine —
...
| `src-tauri/src/protocol.rs` | Answering `calc://` off the bundled `webroot/`                |
...
`cargo test -p calc-shell` runs the entire decision layer on a machine with a
...
**The origin is the one thing to be careful with.** The user's sessions live in
...
`file://` page is an opaque origin, which would leave the sessions unable to
survive a launch.
...
| Variable       | Effect                                                               |
| -------------- | -------------------------------------------------------------------- |
| `CALC_APP_URL` | Load a remote URL instead of the bundled site (the `/preview/` slot) |
| `CALC_WEBROOT` | Serve the site from somewhere else without rebuilding                |
...
identifier (`dev.local.calc`). What an installed copy is called and the
