...
//! shipping: `CALC_APP_URL` points the window at a remote URL instead (the
//! `/preview/` deploy slot, say), and `CALC_WEBROOT` serves a different
...
/// IndexedDB unable to keep a session between launches. A registered scheme
/// gives one stable origin that the sessions are keyed to for the life of the
/// install.
pub const APP_SCHEME: &str = "calc";
...
/// CONSTANT — the origin is what the user's sessions are keyed to, so changing
