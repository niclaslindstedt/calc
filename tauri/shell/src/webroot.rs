...
//! wrong to simplify.
//!
//! The site is served from a registered private scheme (`calc://`, see
...
//! to lose:
//!
//!  1. **One stable origin.** The user's sessions live in IndexedDB and their
//!     settings in `localStorage`, both keyed by origin. `calc://` is a
//!     constant, so the sessions survive every update. A `file://` page (or a
//!     server on an ephemeral port) would hand the user a different origin —
//!     and an empty sessions — at some point.
//!
...
/// `calc:///C:/…` must not become a drive letter.
