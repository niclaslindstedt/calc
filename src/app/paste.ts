...
//
// What the clipboard has to offer the display. A long press on the current
// expression asks this module what — if anything — is worth pasting, and the
// paste button only appears when the answer is not null:
//
//   - text the shared grammar already understands ("12×4.5", "0xFF") pastes
...
//     silent.
//
// The pasted text lands on the display exactly as if it had been typed, so
// everything here stays inside the one grammar every mode shares
// (evaluator.ts). Pure and DOM-free — the clipboard read itself lives in
// CalculatorScreen; the rules below are unit-tested in tests/paste_test.ts.

import { isEvaluable } from "./evaluator.ts";
...
export type PasteCandidate = {
  /** The text that would land on the display. */
  text: string;
  kind: PasteKind;
};
...
// is this app's own decimal separator, so a value copied off the tape must
// paste back as itself ("1.234" is 1.234, but "1.234.567" is grouped).
...
/**
 * What a paste of `clipboard` would put on the display, or null when there is
...
/**
 * What a half of the clipboard bar says it would take or add, cut short when
 * it is longer than a button can honestly show. The paste half names its text
 * because a number salvaged out of prose is not what the user copied — saying
 * so is the difference between a paste and a surprise.
 */
