...
//
// Expression chains. Pressing `=` seeds the next expression with the result,
// so a run of calculations is really one nested expression the user built a
// step at a time:
//
//   1+2 = 3
//   3*2 = 6      ← chained: it starts from the previous result
//
// `chainExpression` folds that run back into a single expression —
...
// Single-character binary operators, in the display spellings the keypad
// emits as well as the ASCII ones (evaluator.ts accepts both).
...
// True when this entry continued from the one before it — the run it belongs
// to has more than one step, so there is a chain worth copying.
export function isChained(entries: readonly Entry[], index: number): boolean {
  return index > 0 && Boolean(entries[index]?.chained);
}

// Fold the run of calculations ending at `index` into one expression. Returns
// null when the entry starts its own run, or when the stored text no longer
// lines up (a hand-edited file) and no faithful chain can be built.
