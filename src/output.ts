...
// `console.*` calls. They fan out to the in-app log buffer (persisted under
// `calc:logs`) so "what did the app just do?" is answerable on-device.

import { logStore } from "./app/log.ts";

const out = logStore.createLogger("app");

/** A normal progress/state line ("Loaded 12 sessions", "Backend connected"). */
