...
import { chainExpression, topLevelPrecedence } from "../src/app/chain.ts";
import { evaluate, formatResult } from "../src/app/evaluator.ts";
import type { Entry } from "../src/app/session.ts";

// Build a tape from `expression` steps, chaining every step after the first
// unless it is written as a bare expression that does not continue the run.
function tape(steps: [expression: string, chained?: boolean][]): Entry[] {
  return steps.map(([expression, chained], index) => {
    const entry: Entry = {
      id: `e${index}`,
      expression,
...
describe("chainExpression", () => {
  ...
  it("gives up when the previous result was typed into a longer number", () => {
    const entries: Entry[] = [
      { id: "a", expression: "1+2", result: "3", at: 0 },
      { id: "b", expression: "35", result: "35", at: 1, chained: true },
    ];
    expect(chainExpression(entries, 1)).toBeNull();
  });
  ...
  it("unwraps a chain step that only pressed = again", () => {
    const entries: Entry[] = [
      { id: "a", expression: "1+2", result: "3", at: 0 },
      { id: "b", expression: "3", result: "3", at: 1, chained: true },
    ];
    expect(chainExpression(entries, 1)).toBe("1+2");
  });
