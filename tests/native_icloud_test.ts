...
describe("the injected provider", () => {
  it("installs itself where the web app looks for it", () => {
    ...
    // `window.__calcICloud`; a rename on either side is not an error, it
    // is a backend that never appears.
    expect(ICLOUD_SCRIPT).toContain("window.__calcICloud");
  });
  ...
  it("offers every method the seam validates, at the version it knows", () => {
    ...
    globals.window = { __calcICloud: host };
    try {
...
describe("isICloudRequest", () => {
  ...
  it("ignores anything the page posts that is not ours", () => {
    ...
    expect(isICloudRequest({ ...good, type: "calc-native/report" })).toBe(
      false,
    );
...
function runScript(script: string): { id: string; result: unknown } | null {
  ...
  const stub = {
    __calcICloudResolve: (id: string, result: unknown) => {
      seen = { id, result };
    },
  };
...
describe("resolveScript", () => {
  it("carries the answer back as parsed JSON, not as a literal", () => {
    ...
    expect(script).toContain("window.__calcICloudResolve");
  });

  it("does not let a session's own text break out of the script", () => {
    // The payload is somebody's saved sessions, and a note is arbitrary user
    // text. This is the case that turns a quote in a label into injected
