...
function withInstalled<T>(value: unknown, body: () => T): T {
  ...
  globals.window = { __calcICloud: value };
  try {
    return body();
  } finally {
...
describe("parseICloudEntries", () => {
  it("keeps the path and the revision token", () => {
    expect(
      parseICloudEntries([{ path: "calculations/budget.md", rev: "17-42" }]),
    ).toEqual([{ path: "calculations/budget.md", rev: "17-42" }]);
  });
  ...
  it("keeps an entry with no revision", () => {
    ...
    expect(parseICloudEntries([{ path: "calculations/a.md" }])).toEqual([
      { path: "calculations/a.md" },
    ]);
  });

  it("drops malformed rows one at a time", () => {
    // One bad row must not cost the reader every session the load would
    ...
    expect(
      parseICloudEntries([
        ...
        null,
        "calculations/b.md",
        { path: "calculations/c.md", rev: 7 },
      ]),
    ).toEqual([{ path: "calculations/a.md" }, { path: "calculations/c.md" }]);
  });
