...
Calc runs in any browser, and it installs to a home screen or a dock as a
...
[releases page](https://github.com/niclaslindstedt/calc/releases).
...
- **macOS** — the `.dmg`. The release notes say whether that release's app
  ...
  scroll to the message about Calc and choose **Open Anyway**. macOS remembers
  after that.
...
around it: the same keypad and layouts, the same settings, the same tape and sessions. Two differences are worth knowing about.
...
the sessions you keep in Chrome are not the ones the desktop app opens. Connect the same folder or Dropbox in both (Settings → **Storage**) and they share one set of sessions.

Uninstalling the app removes that storage with it, so connect a backend or copy out anything you want to keep first.
